...
def main():
    ...
    for zone in set(args.zone):
        ...
        if not cf_zone:
            ...
            r = requests.get(
                API_ENDPOINT + "zones", headers=API_HEADERS, params=payload
            )
            data = r.json().get("result")
            if not data:
                log.critical(
                    "The zone '{}' was not found on your account".format(cf_domain)
                )
                return
...
def get_zone_records(zone_uuid):
    ...
    while current_page != total_pages:
        current_page += 1
        r = requests.get(
            API_ENDPOINT + "zones/" + zone_uuid + "/dns_records?page=" + current_page + "&per_page=50",
            headers=API_HEADERS,
            params=payload,
        )
        data = r.json().get("result")
...
def update_record(zone_record, local_record, resolving_method):
    ...
    if (
        zone_record.get("content") == ip
        and zone_record.get("ttl") == ttl
        and zone_record.get("proxied") == proxied
    ):
    ...
    if not success:
        ...
        return
    log.info(
        "The record '{}' ({}) has been updated successfully".format(name, record_type)
    )
