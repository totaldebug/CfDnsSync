...
def zones(zone_ids: list[str] = None, enabled_only: bool = False, show_subdomains: bool = False,print=logger.info):
    ...
    if zones := factory.zones(zone_ids=zone_ids, enabled_only=enabled_only):
        ...
        records = sum(len(zone.subdomains) for zone in zones)
        print(f"Sub-Domains: {records}")
        if show_subdomains:
            print("==================")
            for zone in zones:
                subs = [sub.name for sub in zone.subdomains]
